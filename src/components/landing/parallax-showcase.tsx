  ...
  return (
    <section ref={containerRef} className="relative py-14 overflow-hidden">
      <motion.div style={{ opacity: sectionOpacity }}>
        ...
        <motion.div style={{ x }} className="flex gap-3 md:gap-4 pointer-events-none">
          {SHOWCASE_ITEMS.map((item, i) => (
            <ShowcaseCard key={i} item={item} index={i} scrollProgress={scrollYProgress} />
          ))}
        </motion.div>
      </motion.div>
    </section>
...
function ShowcaseCard({
  ...
  return (
    <motion.div
      style={{ y }}
      className="flex-shrink-0 w-[40vw] md:w-[28vw] lg:w-[20vw]"
    >
